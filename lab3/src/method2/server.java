...
public class server {
	...
	static ArrayList<ClientSession> ClientSessions = new ArrayList<ClientSession>();
	...
	public static void main(String[] args) throws Exception {
		...
		//create the welcoming server's socket
 	   	...
 	   	new Thread (new Runnable(){ @Override
				...
				ClientSessions.add(new ClientSession(clientCount++, connectionSocket, ClientSessions));
 		  		...
 		  		try {
 		  			...
 		  			while (true) {
 		  				...
 		  				for (int i = 0; i < ClientSessions.size(); i++)
 		  					ClientSessions.get(i).SendMessage("-Count, " + ClientSessions.size() + "\n");
 		  			
 		  				
 		  				//update the label
 		  				if (ClientSessions.size() > 0) //if there are one or more clients print their number
 		 	   			{
 		 	   				if (ClientSessions.size() == 1)
 		 	   					connectionStatusLabel.setText("1 Client Connected");
 		 	   				else
 		 	   					connectionStatusLabel.setText(ClientSessions.size() + " Clients Connected");
	   		...
	   		public void run() {
 		  		...
 		  		try {
 		  			...
 		  			while (true) {
 		  				...
 		  				for (int i = 0; i < ClientSessions.size(); i++)
 		  					ClientSessions.get(i).SendMessage("-Date;" + dateFormatter.format(now) + "\n");
